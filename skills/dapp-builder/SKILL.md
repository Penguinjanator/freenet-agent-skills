...
**Requirement: `get_state_delta` must return zero bytes when the requester's summary already matches your state.** If a peer's summary equals the summary of your state, that peer holds everything you have, and the correct answer is an empty `StateDelta` (`vec![]`). Zero bytes literally: serializing a struct whose fields are all `None` produces 10-15 bytes of encoding framing, which core reads as real content, so the two peers never register as converged and every reconciliation re-ships data the peer already has. Your summary must also be far smaller than your state. Getting either wrong wastes network-wide bandwidth, and core is adding a probe that suppresses contracts which do. The correct and incorrect shapes, and the test that pins them, are in `references/contract-patterns.md` → "The Delta to an Up-to-Date Peer Must Be Zero Bytes".
...
4. Ensure all state updates satisfy the commutative monoid requirement, and that `get_state_delta` returns a zero-byte delta when the requester's summary already matches your state (see `contract-patterns.md`)
